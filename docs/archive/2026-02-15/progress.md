...
- [x] **Granular Notification UI**: 앱 푸시, 카카오톡, 이상 징후, 리포트 알림 상세 설정 UI 구현
- [x] **Database Schema Expansion**: 유저별 알림 환경설정 및 FCM 토큰 저장을 위한 `notification_settings` 테이블 생성
- [x] **Backend Settings API**: 설정을 조회/저장(Upsert)하기 위한 FastAPI 엔드포인트 구축
- [x] **Server State Synchronization**: TanStack Query를 이용한 설정 페이지 및 대시보드 동기화
- [x] **Dashboard QuickActions**: 홈 화면에서 즉시 알림을 끄고 켤 수 있는 인터랙션 추가
- [x] **Permission Flow**: PWA 환경에서의 브라우저 알림 권한 요청 로직 적용

---
## ⏳ Phase 22: Notification Delivery & Logic (Pending)
- [ ] **FCM/VAPID Configuration**: Firebase 연동 및 Push 알림용 VAPID 인증 키 설정
- [ ] **Service Worker Implementation**: 앱 백그라운드 구동 시 알림 수신을 위한 `sw.ts` 엔진 구축
- [ ] **Anomaly Trigger Integration**: 설비 데이터 분석 알고리즘과 알림 서버 간의 실시간 트리거 연결
- [ ] **AI Report Batch Scheduler**: 매일 자동 리포트 생성 및 알림 발송을 위한 스케줄링 시스템 구축

---
*마지막 업데이트: 2026-02-06 (10:52 AM)*
